...
export const validateAndCleanRow = (row) => {
  ...
  // This handles cases where data only has StoreName
  if (!storeCode) {
    if (!storeName) {
      return null; // Both StoreCode and StoreName are empty, invalid row
    }
    storeCode = storeName; // Use StoreName as StoreCode
  }
 ...
 * Two rows are considered duplicates if all fields (StoreName, StoreCode, Amount, Hour, Day, Date) match exactly
 *
 * @param {Array} data - Raw data array
 * @returns {Object} - { uniqueData: Array, duplicateCount: number }
 */
export const removeDuplicateRows = (data) => {
  ...
  for (const row of data) {
    // Create a unique key from all data fields
 ...
 *
 * @param {Array} data - Raw data array with Date field
 * @param {string} fromDate - Start date in YYYY-MM-DD format (empty = no filter)
 * @param {string} toDate - End date in YYYY-MM-DD format (empty = no filter)
 * @returns {Array} - Filtered data
 */
...
/**
 * Format hour range to AM/PM format
 *
 * @param {number} startHour - Start hour in 24-hour format (0-23)
 * @param {number} endHour - End hour in 24-hour format (0-23, or 24 for midnight)
 * @returns {string} - Formatted time range (e.g., "9 AM - 10 AM", "2 PM - 3 PM", "11 PM - 12 AM")
 */
export const formatHourRangeToAMPM = (startHour, endHour) => {
  // Handle endHour = 24 (midnight)
  const normalizedEndHour = endHour === 24 ? 0 : endHour;
  return `${formatHourToAMPM(startHour)} - ${formatHourToAMPM(normalizedEndHour)}`;
};

...
const generateAmount = (store, day, hour) => {
  ...
  if (hour >= 11 && hour <= 13) hourMultiplier = 1.4;      // Lunch rush
  else if (hour >= 17 && hour <= 19) hourMultiplier = 1.5; // Dinner rush
  else if (hour < 10) hourMultiplier = 0.5;                 // Early morning slow
  ...
  else if (day === 'Monday') dayMultiplier = 0.8;          // Monday blues
  ...
  const amount = baseAmount * storeMultiplier * hourMultiplier * dayMultiplier * randomFactor;
